...
import { useRealtimeMetrics, useAPISubscription } from '@/providers/RealtimeProvider';
...
const LiveMetrics: React.FC<LiveMetricsProps> = ({
  ...
  // Subscribe to all APIs for real-time metrics
        ...
        apiMetrics.apiId === update.apiId
          ? {
              ...
              responseTime: {
                ...
                trend: calculateTrend(apiMetrics.responseTime.value, update.metrics.responseTime),
              },
              uptime: {
                value: update.metrics.uptime,
                timestamp: update.metrics.timestamp,
                trend: calculateTrend(apiMetrics.uptime.value, update.metrics.uptime),
              },
              errorRate: {
                value: update.metrics.errorRate,
                timestamp: update.metrics.timestamp,
                trend: calculateTrend(apiMetrics.errorRate.value, update.metrics.errorRate, true),
              },
              requestCount: {
                value: update.metrics.requestCount,
                timestamp: update.metrics.timestamp,
                trend: calculateTrend(apiMetrics.requestCount.value, update.metrics.requestCount),
              },
  ...
  const calculateTrend = (oldValue: number, newValue: number, inverse = false): 'up' | 'down' | 'stable' => {
    ...
    switch (trend) {
      case 'up':
        return (
          <TrendingUp
            {...iconProps}
            className={clsx('w-3 h-3', inverse ? 'text-red-500' : 'text-green-500')}
          />
        );
      case 'down':
        return (
          <TrendingDown
            {...iconProps}
            className={clsx('w-3 h-3', inverse ? 'text-green-500' : 'text-red-500')}
          />
        );
      case 'stable':
  ...
  const isAlertTriggered = (metric: MetricValue, threshold: number, inverse = false) => {
    return inverse ? metric.value > threshold : metric.value < threshold;
  };

  const formatValue = (value: number, type: 'time' | 'percentage' | 'count') => {
    switch (type) {
      case 'time':
    ...
    <div className={clsx('space-y-6', className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            <div className={clsx('w-2 h-2 rounded-full', isLive ? 'bg-green-500 animate-pulse' : 'bg-gray-400')} />
        ...
        {metrics.map((apiMetrics) => (
          <div
            ...
            className={clsx(
              ...
              onAPIClick ? 'cursor-pointer hover:shadow-lg hover:border-blue-300' : 'cursor-default'
            )}
          >
            {/* API Header */}
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-lg font-medium text-gray-900">{apiMetrics.apiName}</h4>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {apiMetrics.responseTime.trend && getTrendIcon(apiMetrics.responseTime.trend)}
                </div>
                <div className="flex items-center space-x-2">
                  <span className={clsx(
                    'text-lg font-semibold',
                    isAlertTriggered(apiMetrics.responseTime, alertThresholds.responseTime, true)
                      ? 'text-red-600'
                      : 'text-gray-900'
                  )}>
              ...
              <div className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {apiMetrics.uptime.trend && getTrendIcon(apiMetrics.uptime.trend)}
                </div>
                <div className="flex items-center space-x-2">
                  <span className={clsx(
                    'text-lg font-semibold',
              ...
              <div className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {apiMetrics.errorRate.trend && getTrendIcon(apiMetrics.errorRate.trend, true)}
                </div>
                <div className="flex items-center space-x-2">
                  <span className={clsx(
                    'text-lg font-semibold',
                    isAlertTriggered(apiMetrics.errorRate, alertThresholds.errorRate, true)
                      ? 'text-red-600'
                      : 'text-gray-900'
                  )}>
              ...
              <div className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {apiMetrics.requestCount.trend && getTrendIcon(apiMetrics.requestCount.trend)}
                </div>
      ...
      {metrics.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">Start monitoring APIs to see real-time metrics.</p>
        </div>
      )}
    </div>
  );
...
export const LiveMetricsSummary: React.FC<LiveMetricsSummaryProps> = ({
  ...
  const avgResponseTime = metrics.reduce((sum, m) => sum + m.responseTime.value, 0) / metrics.length;
  const avgUptime = metrics.reduce((sum, m) => sum + m.uptime.value, 0) / metrics.length;
  const avgErrorRate = metrics.reduce((sum, m) => sum + m.errorRate.value, 0) / metrics.length;
  const totalRequests = metrics.reduce((sum, m) => sum + m.requestCount.value, 0);
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="text-center">
          ...
          <p className="text-lg font-semibold text-gray-900">
            {avgResponseTime < 1000 ? `${avgResponseTime.toFixed(0)}ms` : `${(avgResponseTime / 1000).toFixed(1)}s`}
          </p>
        ...
        <div className="text-center">
          ...
          <p className="text-lg font-semibold text-gray-900">{avgUptime.toFixed(1)}%</p>
          <p className="text-xs text-gray-600">Avg Uptime</p>
        </div>
        
        <div className="text-center">
          <AlertTriangle className="w-6 h-6 mx-auto mb-2 text-red-500" />
          <p className="text-lg font-semibold text-gray-900">{avgErrorRate.toFixed(1)}%</p>
