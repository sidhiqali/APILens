...
const HealthScoreCard: React.FC<HealthScoreCardProps> = ({
  ...
  const scoreBreakdown = useMemo(() => {
    const totalWeight = data.metrics.reduce((sum, metric) => sum + metric.weight, 0);
    const weightedScore = data.metrics.reduce((sum, metric) => sum + (metric.value * metric.weight), 0);
  ...
  const getScoreStatus = (score: number) => {
    ...
    if (score >= 70) return { status: 'warning', color: 'yellow', label: 'Warning' };
  ...
  const getMetricIcon = (metricName: string) => {
    ...
    if (name.includes('uptime') || name.includes('availability')) return TrendingUp;
    if (name.includes('error') || name.includes('failure')) return AlertTriangle;
    if (name.includes('throughput') || name.includes('requests')) return Activity;
  ...
  // Render metric bar
  const MetricBar = ({ metric }: { metric: HealthMetric & { contribution: number; normalizedWeight: number } }) => {
          ...
          <div className="flex items-center space-x-2">
            ...
            {showTrends && (
              <div className="flex items-center">
                ...
                {metric.trend === 'stable' && <Activity className="w-3 h-3 text-gray-600" />}
              </div>
            )}
          </div>
          <div className="flex items-center space-x-2">
    ...
    <div className={clsx(
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900">{data.apiName}</h3>
        ...
        <div className="flex items-center space-x-4">
          {scoreChange && (
            <div className={clsx(
              ...
              <span>{scoreChange.isPositive ? '+' : ''}{scoreChange.value.toFixed(1)}</span>
            </div>
          )}
          <CircularProgress score={data.overallScore} size={size === 'small' ? 60 : size === 'large' ? 100 : 80} />
        </div>
      </div>
      ...
      {showDetails && (
        <div className="space-y-4">
          ...
          {/* Score Breakdown */}
          <div className="border-t pt-4">
            <h4 className="text-sm font-semibold text-gray-900 mb-3">Score Composition</h4>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-gray-600">Calculated Score:</span>
                <span className="font-medium ml-2">{scoreBreakdown.calculated.toFixed(1)}</span>
              </div>
              <div>
                <span className="text-gray-600">Reported Score:</span>
                <span className="font-medium ml-2">{data.overallScore.toFixed(1)}</span>
              </div>
              <div>
                <span className="text-gray-600">Variance:</span>
                <span className={clsx(
                  'font-medium ml-2',
                  scoreBreakdown.variance > 5 ? 'text-red-600' : 'text-green-600'
                )}>
              ...
              <div>
                ...
                <span className="font-medium ml-2">{data.slaCompliance.toFixed(1)}%</span>
              </div>
            </div>
          </div>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                ...
                <ul className="text-sm text-red-700 space-y-1">
                  {data.metrics
                    ...
                    .map((metric, index) => (
                      <li key={index}>
                        • {metric.name}: {metric.value.toFixed(1)} (target: {metric.target})
                      </li>
                    ))
                  }
                </ul>
      ...
      {!showDetails && (
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <div className="text-sm font-semibold text-gray-900">{data.metrics.length}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="text-sm font-semibold text-gray-900">{data.slaCompliance.toFixed(0)}%</div>
          </div>
        </div>
      )}
    </div>
